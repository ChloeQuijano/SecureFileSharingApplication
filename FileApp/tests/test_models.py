...
class SharedFileModelTestClass(TestCase):
    ...
    def setUp(self):
        ...
        self.user_sharedto = User.objects.create_user(username='testuser2', password='testpassword2')

        # Create a test shared file instance
        content = b'Test file content'
        file_content = ContentFile(content, name='test_file.txt')
        test_file = File.objects.create(owner=self.user, file=file_content, file_name='Test file', file_size=file_content.size)

        self.test_sharedfile = SharedFile.objects.create(user=self.user_sharedto, file=test_file, permission='edit', file_name=test_file.file_name)
